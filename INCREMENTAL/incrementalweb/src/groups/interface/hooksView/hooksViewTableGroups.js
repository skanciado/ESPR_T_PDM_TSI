...
export default function HookViewTableGroups(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsGroup", {replace: true});
      let sExtendedTable = "tblGroups";
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle="Detalles" twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormGroups} idFormBody={formState.formValues.id} setModal={setModal} initialModalValues={formState.formValues} />}
    </>
  );
}
