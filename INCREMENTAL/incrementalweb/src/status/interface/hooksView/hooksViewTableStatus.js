...
export default function HookViewTableStatus(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsStatus", {replace: true});
      let sExtendedTable = "tblStatus";
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues?.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormStatus} idFormBody={formState.formValues?.id} setModal={setModal} initialModalValues={formState.formValues} />}
    </>
  );
}
