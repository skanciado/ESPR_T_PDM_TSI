...
import {findStatus} from "../../../status/infrastructure/services/bdsStatus";
import {getInitialStatus} from "../../../utilities/application/utilsApplication";
import {FOLDER_OBJECT_TYPE} from "../../../utilities/allLayers/utilsAllLayers.js"
export default function HookViewTableFolders(props) {
  const context = useContext(Context);
  const [formState] = useState({
    formValues: [],
    formAction: undefined,
    myUpdateFunc: undefined,
  });
  ...
  const eventHandlerOnClick_ButtonsTableBar = async (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "create") {
      //recuperamos lifecycle para objectype
      let resultInitStatus = await getInitialStatus(FOLDER_OBJECT_TYPE);
      //TODO: Si hay varios estados iniciales hay que mostrar una selección
      let resultStatus = await findStatus(resultInitStatus[0]);

      elementsForm.status  = resultStatus.name;
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsFolder", {replace: true});
      let sExtendedTable = "tblFolders";
  ...
  if (modalState.valuesModal !== undefined && JSON.stringify(modalState.valuesModal) !== "{}") {
    ...
    if (formState.formAction === "processingCreate") {
      formState.formValues._id = undefined;
      //guardar en bds despues de cerrar el modal el valor de modalState.valuesModal
      props.crudApTableSave(context, "create", formState.formValues._id, modalState.valuesModal, props.dialogContent.attributes).then( ( result ) => {
    ...
    if (formState.formAction === "processingUpdate") {
      //guardar en bds despues de cerrar el modal el valor de modalState.valuesModal
      props.crudApTableSave(context, "update", formState.formValues._id, modalState.valuesModal, props.dialogContent.attributes).then( ( result ) => {
        if( formState.myUpdateFunc !== undefined) {
            formState.myUpdateFunc(result);
        }
      });
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormFolders} idFormBody={formState.formValues.id} setModal={setModal} initialModalValues={formState.formValues} />}
    </>
  );
}
