...
export default function HookViewTableProjects(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = async (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "create") {
      ...
      let resultInitStatus = await getInitialStatus(PROJECT_OBJECT_TYPE);
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsProject", {replace: true});
      let sExtendedTable = "tblProjects";
    ...
    if (idButton === "import") {
      let element = document.getElementById("openModalImportFile");
  ...
  if (modalState.valuesModal !== undefined && JSON.stringify(modalState.valuesModal) !== "{}") {
    ...
    if (formState.formAction === "processingCreate") {
      ...
      //guardar en bds despues de cerrar el modal el valor de modalState.valuesModal
       ...
       .then( ( result ) => {
            //getValuesModalItem('Projects', context);
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues?.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormProjects} idFormBody={idForm} setModal={setModal} initialModalValues={formState.formValues} />}      
    </>
  );
}
