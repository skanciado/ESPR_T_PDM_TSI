...
import {findAllDocuments, createDocument, deleteDocument, updateDocument} from "../../infrastructure/services/bdsDocument";
...
import {  DOCUMENT_OBJECT_TYPE } from "../../../utilities/allLayers/utilsAllLayers";
...
export default async function crudApTableDocument(props) {
  ...
  //let tableValues = await findAllDocumentsForUser(sCurrentUserId, sCurrentGroupId, sCurrRoleId, bUserIsAdmin);
  //obtengo los valores de la tabla Documents en caso de tener que editar datos, en caso contrario values={undefined}
  let tableValues = await findAllDocuments();
  //obtengo los atributos de la tabla y el modal
  let dialogContent = await attributelistObjectTypes(DOCUMENT_OBJECT_TYPE);
  //traduzco las labels
  ...
  if (props?.isSearcher !== true) {
    props.context.tableDispatch.handlTableCreateInArr({table: "documents", dialogContent: dialogContentAdvancedSearch, valuesAllSelectOne: arraysWithWhite});
  }
  //obtenemos el status (objectType Base)
  tableValues = await getStatus(tableValues, DOCUMENT_OBJECT_TYPE);
...
export async function crudApTableDocumentsSave(context, action, dbId, values, attributes) {
  try {
    ...
    if (action === "create") {
      resData = await createDocument(savedValues);
      if (resData !== undefined) {
        resStatus = await createRelations(resData._id, relationKeys, documentGroupRelations, DOCUMENT_OBJECT_TYPE);
        //Update at least _id
        values['_id'] = resData._id;
      }
    }
    if (action === "update" && dbId !== undefined) {    
      resData  = await updateDocument( dbId, savedValues);
      if (resData !== undefined) {
        resStatus = await updateRelations(dbId, relationKeys, documentGroupRelations, DOCUMENT_OBJECT_TYPE);
      }
      //Update at least _id
      values['_id'] = dbId;
    }
    if (action === "delete" ) {
      for await (const id of values[values.length - 1]) {
        ...
        resData = await deleteDocument(id);
      }
    }
    if (resData === 0 && resStatus !== 0) {
      errorMessage("crudApTableDocumentsSave", "Error data");
      return false;
    } else {
      return values;
    }
  } catch (e) {
    errorMessage("crudApTableDocumentsSave", e);
  }
}
