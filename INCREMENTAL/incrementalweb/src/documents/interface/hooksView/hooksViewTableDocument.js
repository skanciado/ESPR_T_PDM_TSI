...
import {DOCUMENT_OBJECT_TYPE} from "../../../utilities/allLayers/utilsAllLayers.js"
export default function HookViewTableDocuments(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = async (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "create") {
      ...
      let resultInitStatus = await getInitialStatus(DOCUMENT_OBJECT_TYPE);
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsDocument", {replace: true});
      let sExtendedTable = "tblDocuments";
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormDocuments} idFormBody={formState.formValues.id} setModal={setModal} initialModalValues={formState.formValues} />}
    </>
  );
}
