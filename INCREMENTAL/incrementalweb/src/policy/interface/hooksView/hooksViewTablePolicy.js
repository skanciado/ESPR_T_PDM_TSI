...
export default function HookViewTablePolicies(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsPolicy", {replace: true});
      let sExtendedTable = "tblPolicies";
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues?.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormPolicies} idFormBody={formState.formValues?.id} setModal={setModal} initialModalValues={formState.formValues} />}
    </>
  );
}
