...
import HookViewLifecycleGraph from "./hookLifecycleGraph/hookViewLifecycleGraph";

export default function ButtonHookViewLifecycleGraph(){
    ...
    <>
      <button
        ...
        onClick={async (e) => {
          ...
          const sStatusDbId   = aParamsData[5];

          const dataAndDesing = await getValuesModalItem2(sTableName, sObjectDbId, sStatusDbId, context);
          if (dataAndDesing !== undefined) {
            setModal({
              openModal: true,
              valuesModal: undefined,
              attributes: {tableForm: sTableName, cellForm: sPropName, cellChange: sPropId, cellType: sPropValType, dbId: sStatusDbId},
        ...
        hidden
      />
      {modal.openModal && <HookViewLifecycleGraph lblTitle="Current Lifecycle" twoButtons={true} lblCancel="Cancelar" lblOk="OK" idFormBody={""} setModal={setModal} modal={modal} /> }
    </>
  );
}