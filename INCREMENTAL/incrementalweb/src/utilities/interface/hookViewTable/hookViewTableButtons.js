export default function HookViewTableButtons(props) {
  const resultButtons = Object.entries(props.buttonsConfig).map(([key, value], c) => {
    return <div key={c}>{returnButtonsComponent(props.id, key, value.readOnly, value.src, value.path, value.label, value.width, value.height, props.eventHandlerOnClickButtonsBar)}</div>;
  });
  return (
    <div id={props.id + "_btnBar"} className="flex px-5 py-5 text-sm">
      {resultButtons}
    </div>
  );
}
