...
export default function HookViewTableTasks(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = (idButton, stateTable, updateFunc) => {
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsTask", {replace: true});
      let sExtendedTable = "tblTasks";
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues?.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormTasks} idFormBody={formState.formValues?.id} setModal={setModal} initialModalValues={formState.formValues} />}
    </>
  );
}
