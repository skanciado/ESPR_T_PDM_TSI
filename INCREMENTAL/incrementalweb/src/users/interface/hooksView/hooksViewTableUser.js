...
import {USER_OBJECT_TYPE} from "../../../utilities/allLayers/utilsAllLayers.js"
export default function HookViewTableUsers(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = async (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "create") {
       ...
       let resultInitStatus = await getInitialStatus(USER_OBJECT_TYPE);
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsUser", {replace: true});
      let sExtendedTable = "tblUsers";
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues?.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormUsers} idFormBody={formState.formValues?.id} setModal={setModal} initialModalValues={formState.formValues} />}
