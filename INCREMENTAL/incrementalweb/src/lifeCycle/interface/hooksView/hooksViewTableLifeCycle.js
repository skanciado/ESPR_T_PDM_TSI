...
export default function HookViewTableLifeCycles(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = async (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsLifeCycle", {replace: true});
      let sExtendedTable = "tblLifeCycles";
  ...
  return (
    <>
      ...
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues?.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormLifeCycles} idFormBody={formState.formValues?.id} setModal={setModal} initialModalValues={formState.formValues} />}
    </>
  );
}
