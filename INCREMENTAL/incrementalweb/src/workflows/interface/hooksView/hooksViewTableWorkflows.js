...
export default function HookViewTableWorkflows(props) {
  ...
  const eventHandlerOnClick_ButtonsTableBar = (idButton, stateTable,updateFunc) => {
    ...
    if (idButton === "expand") {
      //navigate("/home/detailsWorkflow", {replace: true});
      let sExtendedTable = "tblWorkflows";
  ...
  if (context.filtersTable !== undefined && context.filtersTable.table === "workflows" && context.filtersTable.filters !== undefined) {
    tableFilters = context.filtersTable.filters;
    context.filtersTable.filters = undefined;
    context.filtersTable.table = undefined;
  }
  const HookViewFormWorkflows = () => <HookViewForm idForm={formState.formValues?.id} dialogContent={props.dialogContent.attributes} valuesForm={formState.formValues} valuesAllSelectOne={props.valuesAllSelectOne} eventHandlerOnClickButtons={eventHandlerOnClick_ButtonsForm} action={formState?.formAction} valuesAction={{user: context.user._id, group: context.cache.groupLast}}/>;
  return (
    <>
      <HookViewTable id={"tblWorkflows"} title="workflows" dialogContent={props.dialogContent} values={props.tableValues} showLabels="true" valuesAllSelectOne={props.valuesAllSelectOne} eventHandlerOnClickButtonsTableBar={eventHandlerOnClick_ButtonsTableBar} eventHandlerOnClickButtonsTableBody={eventHandlerOnClick_ButtonsTableBody} eventHandlerModalValues={modalState.valuesModal} eventHandlerOnChange_Filter={tableFilters} />
      {modalState.openModal && <HookViewModal lblTitle={"Información " + formState.formValues.name} twoButtons={true} lblCancel="Cancel" lblOk="Save" ctrlBody={HookViewFormWorkflows} idFormBody={formState.formValues.id} setModal={setModal} initialModalValues={formState.formValues} />}
    </>
  );
}
